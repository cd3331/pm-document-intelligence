...
class ReportGenerator:
    ...
    async def generate_and_send_report(
        ...
        try:
            app_logger.info(
                f"Generating {report_type} report in {format} format",
                user_id=user_id
            )
            ...
            await self._send_report_email(
                email_to, report_type, report_file, format
            )
    ...
    async def _collect_report_data(
        ...
        # User activity
        active_users = self.db.query(func.count(func.distinct(Document.user_id))).filter(
            Document.created_at.between(start_date, end_date)
        ).scalar()
        ...
        total_cost = sum(
            entry.total_cost for entry in cost_tracker.cost_history
    ...
    async def generate_excel_report(
        ...
        try:
            ...
            if data.get('time_series'):
                ...
                data_ref = Reference(ws_timeseries, min_col=2, min_row=1, max_row=len(data['time_series'])+1)
                cats = Reference(ws_timeseries, min_col=1, min_row=2, max_row=len(data['time_series'])+1)
