...
async def get_documents_list(
    ...
    for doc in documents:
        ...
        created_at = doc.created_at.strftime("%b %d, %Y at %I:%M %p") if doc.created_at else "Unknown"
...
async def get_document_actions(
    ...
    action_items = db.query(ActionItem).filter(
        ...
        ActionItem.user_id == current_user.id
    ).order_by(ActionItem.priority.desc(), ActionItem.created_at).all()
...
async def get_processing_status(
    ...
    for doc in processing_docs:
        ...
        current_step = doc.metadata.get("current_step", "Processing") if doc.metadata else "Processing"
...
async def search_documents(
    ...
    try:
        if semantic:
            ...
            # Apply search
            query = query.filter(Document.filename.ilike(f"%{q}%") | Document.extracted_text.ilike(f"%{q}%"))
