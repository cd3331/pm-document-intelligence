...
async def get_document_stats(
    ...
    with track_request_duration(method="GET", endpoint="/analytics/documents/stats"):
        try:
            ...
            # Documents by status
            status_counts = (
                query.with_entities(Document.status, func.count(Document.id))
            ...
            # Documents by type
            type_counts = (
                query.with_entities(Document.document_type, func.count(Document.id))
...
async def get_cost_optimization(
    ...
    with track_request_duration(method="GET", endpoint="/analytics/costs/optimization"):
        try:
            ...
            recent_openai_costs = [
                entry
                for entry in cost_tracker.cost_history[-1000:]
            ...
            documents_processed_last_30_days = (
            ...
            cost_per_document = (
                (total_cost_last_30_days / documents_processed_last_30_days)
                if documents_processed_last_30_days > 0
                else 0
            )
...
async def get_api_performance(
    ...
    with track_request_duration(method="GET", endpoint="/analytics/performance/api"):
        try:
            ...
            return {
                "request_volume": {"total": 12543, "success": 12234, "error": 309},
                "latency": {"p50": 0.125, "p95": 0.543, "p99": 1.234},
                "error_rate": 2.46,
                "slowest_endpoints": [
...
async def update_dashboard_config(
    ...
    return {"message": "Dashboard configuration updated", "config": config}
