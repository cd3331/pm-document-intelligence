...
from sqlalchemy import Column, String, DateTime, Date, JSON, Integer, Float, Boolean, Index
...
class AnalyticsSnapshot(Base):
    ...
    metric_type = Column(String(50), nullable=False, index=True)  # documents, users, costs, performance
    data = Column(JSONB, nullable=False)  # Flexible JSON storage for metrics
    created_at = Column(DateTime, default=datetime.utcnow, nullable=False)

    # Composite index for efficient querying
    __table_args__ = (
        Index('idx_snapshot_date_type', 'date', 'metric_type'),
    )
...
class AnalyticsEvent(Base):
    ...
    event_type = Column(String(100), nullable=False, index=True)  # page_view, button_click, feature_usage
