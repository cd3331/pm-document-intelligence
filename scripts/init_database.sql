...
-- Users can read analysis for their own documents
CREATE POLICY analysis_select_own ON analysis
    FOR SELECT
    USING (auth.uid() = user_id);

-- System can insert analysis (typically done by backend)
CREATE POLICY analysis_insert_system ON analysis
    FOR INSERT
    WITH CHECK (auth.uid() = user_id);

-- Users can update analysis for their own documents
CREATE POLICY analysis_update_own ON analysis
    FOR UPDATE
    USING (auth.uid() = user_id)
    WITH CHECK (auth.uid() = user_id);

-- Users can delete analysis for their own documents
...
SELECT
    ...
    SUM(json_array_length(action_items)) AS total_action_items,
    SUM(json_array_length(risks)) AS total_risks,
